    ...
    render() {
        ...
        return(
            <div className='form-inline my-2 my-lg-0'>
                <input onKeyPress={(event) => this.handleKeypress(event)} ref={this.searchInput} className='form-control mr-sm-2 bwm-search' 
                       type='search' placeholder='Try New York' 
                       aria-label='Search'></input>
                <button onClick={this.handleSearch} className='btn btn-outline-success my-2 my-sm-0 btn-bwm-search' type='submit'>
                    Search
                </button>
            </div>
        );
